...
- **Field manual described Overload M-Drive incorrectly** — the Actions-table row said "8+ (Engineer) — +Effect Thrust available this round *(CRB p.167)*". RAW (*CRB p.171*) is a Difficult **10+** check granting a **fixed +1** Thrust on the **next** round, not an Effect-scaled bonus this round — p.167 is an unrelated section (Repair System / Reload Turret). The app code already matched RAW correctly (`crewActions.js` difficulty 10, `ActionModal.jsx` fixed `+1`); only the documentation was wrong. Found while doc-syncing the two fixes above.
  (`doc/field-manual.md`)
...
- **#17 — Quad Turrets (HG p.81)** — Ship profiles now support up to 4 weapons per turret slot. The profile builder shows a live SINGLE / DOUBLE / TRIPLE / QUAD label as weapons are added. The Ship Detail modal labels quad slots as "Quad Turret". Combat resolution is unchanged: the existing same-type linking formula already yields +3 bonus damage dice for 4 weapons, and the Point Defence DM formula (`weaponCount − 1`) already produces DM+3 for 4 lasers — matching HG p.81 RAW. (`ShipProfileForm.jsx`, `ShipDetailModal.jsx`)
