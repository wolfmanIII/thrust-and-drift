...
- **Ion weapon naming — RAW disambiguation** — HG uses two different names for these weapons depending on chapter context. Standard space combat chapter (HG pp.29–30, 32–33): **Ion Cannon** (barbette) and **Ion Cannon Bay** (bay). Fleet Combat chapter (HG p.112): *Ion Barbette*, *Small Ion Bay*, *Medium Ion Bay*, *Large Ion Bay* — names specific to the fleet combat abstraction, which uses a wholly different damage system (effect-per-weapon × count ÷ Hull Points → Ion Damage table; no Power stat, no thrust formula). T&D implements standard space combat only. "Ion Cannon" and "Ion Cannon Bay" are the correct canonical names for that context and will not change.
