...
function CommandConsole({ mode, onModeChange, onNewSession, onResumeClick, onResumeAutosave, onClearAutosave, onHelp, onChangelog, autosave, loading, error }) {
  return (
    <div className="border-r border-slate-800 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 px-5 py-5 space-y-5 overflow-y-auto">
        ...
        <div>
          <p className="font-display text-xs text-slate-400 tracking-widest mb-2">COMBAT MODE</p>
          <div className="grid grid-cols-2 gap-1 p-1 bg-slate-900/60 border border-slate-800 rounded-lg">
            {[
              ...
              >
                {label}
                <span className="block font-mono tracking-normal normal-case text-slate-400 mt-0.5 text-xs">{sub}</span>
              </button>
            ))}
          </div>
        </div>
        ...
        <div>
          <p className="font-display text-xs text-slate-400 tracking-widest mb-2">ACTIONS</p>
    ...
    <div className="relative flex flex-col h-full overflow-hidden bg-slate-950">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="px-6 py-3 border-b border-slate-800/60 shrink-0 flex items-center gap-3">
          ...
          <span className="font-display text-xs text-slate-400 tracking-widest">STANDBY</span>
        </div>
        ...
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center space-y-5">
            ...
            <div className="space-y-1">
              ...
              <p className="font-mono text-xs text-slate-400">Start a new session</p>
              <p className="font-mono text-xs text-slate-400">or load a previous session</p>
            </div>
          </div>
        </div>
    ...
    <div className="relative flex flex-col h-full overflow-hidden bg-slate-950">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="flex-1 overflow-y-auto px-6 py-4">
          ...
          {ships.length === 0 && (
            <p className="font-mono text-xs text-slate-400">No ships on record.</p>
          )}
...
function DataField({ label, value, accent = false, small = false }) {
  return (
    <div>
      <p className="font-display text-xs text-slate-400 tracking-widest leading-none mb-0.5">{label}</p>
...
function ShipPreviewRow({ ship }) {
  ...
  return (
    <div className="flex items-center gap-2.5">
      ...
      <span className="font-mono text-xs text-slate-400 w-10 text-right shrink-0">
      ...
      <div className="relative z-10 flex flex-col h-full">
        ...
        <div className="shrink-0 px-6 py-4 border-t border-slate-800 space-y-2">
          ...
          <button
            onClick={onCancel}
            disabled={loading}
            className="w-full py-2 border border-slate-700 text-slate-400 font-display text-xs tracking-widest rounded-lg hover:border-slate-600 hover:text-slate-400 transition-colors disabled:text-slate-400 disabled:border-slate-600/50 disabled:bg-transparent"
          >
            CANCEL
          </button>
        </div>
  ...
  return (
    <div className="h-full grid grid-cols-[340px_1fr] overflow-hidden">
      <CommandConsole
        ...
        onChangelog={() => gotoScreen('changelog')}
        autosave={autosave}
        loading={loading}
        error={error}
      />
    ...
    <div className="w-full h-full flex bg-slate-950">
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <header className="shrink-0 px-6 py-4 border-b border-slate-800 flex items-center gap-4">
          ...
          <span className="ml-auto text-slate-400 font-mono text-xs">v1.20.7</span>
        </header>
