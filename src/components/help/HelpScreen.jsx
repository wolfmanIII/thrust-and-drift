      ...
      <main className="help-content flex-1 overflow-y-auto px-8 py-6 space-y-10">
        ...
        <Section id="setup" title="Setup Phase">
          ...
          <Sub title="ADDING A SHIP">
            ...
            <KV k="Initial vector" v="Vectorial mode only. Pre-set the ship's starting velocity vector (default 0 / 0). Use for ships arriving at cruise speed, fleeing, or intercepting at scenario start — see below for how the compass works." />
          ...
          <Sub title="SETTING THE INITIAL VECTOR">
            <KV k="Direction buttons" v="Six buttons (NW/N/NE/SW/S/SE) surround a center preview. Each click adds exactly one hex-step in that direction to the current vector — it's cumulative, not a single-choice selector." />
            <KV k="Straight-line vector" v="Click the same button N times for a vector of N hexes in that direction (e.g. click NE three times for 3 hexes northeast)." />
            <KV k="E / W buttons" v="Either side of the center preview. A flat-top hex has no true East/West neighbour — only the 6 directions above exist as a single hex-step — so E/W are composite shortcuts: one click applies two hex-steps at once (E = NE+SE = (+2,-1), W = NW+SW = (-2,+1)), same net result as clicking both in sequence." />
            <KV k="Center preview" v="Shows the chosen ship shape/colour inside a hex frame, rotated live to always face the vector's actual current direction. Use it — and the (q, r) readout underneath — to confirm the result after every click." />
            <KV k="Reset" v="Click the center preview itself (the ship icon, marked with a small ↺) to set the vector back to (0, 0) immediately, regardless of how many clicks came before." />
            <Note>Clicking different directions in sequence does not "switch" the heading — it adds up. Each click is a hex-step added to whatever is already set, like axial vector addition; alternating between two directions composes a diagonal result and does not cancel earlier clicks. For a clean single-direction vector, click only that one button, repeatedly. For a diagonal vector on purpose, mixing directions is exactly right — just watch the rotating preview and the (q, r) readout to confirm the composite result before confirming placement. Unrelated to the Thrust budget used in the Acceleration phase — there is no cap and no cost here, it's purely a starting-condition convenience. The Δq/Δr fields below the compass always show (and accept) the exact current values, if typing the vector directly is easier than reasoning about clicks.</Note>
          </Sub>
        ...
        <Section id="attack" title="Attack Phase">
          ...
          <Sub title="LAUNCHING MISSILES">
            ...
            <p><span className="text-slate-200">Missile Rack</span> (CRB p.162) — adjust salvo count using the stepper. Salvo size is capped at the number of Missile Rack components mounted in the selected turret — a lone rack sharing a turret with other weapon types launches at most 1 missile per round, a homogeneous triple-rack turret up to 3 (Traveller Companion p.167 — "a double missile rack turret can still only fire two missiles at once"). Magazine: 12 per rack. Damage: 4D6 per missile.</p>
            <p><span className="text-slate-200">Missile Barbette</span> (HG p.29–30) — RAW fires a fixed 5-missile salvo; the simulator offers a 1–5 stepper instead as an optional house-rule (finishing off a damaged target, warning shots, probing PD, conserving ammo). 25 total canisters. Damage: 4D6 per missile. Barbette ×3 multiplier does <em>not</em> apply to missile weapons.</p>
            <p><span className="text-slate-200">Torpedo</span> (HG p.30–31) — 1–3 torpedoes per launch, 3 per barbette. Rendered as a red/amber token. Damage: 6D6 per torpedo. Torpedo ammo is tracked in a <span className="text-slate-200">separate pool</span> from missile ammo — the two magazines are independent.</p>
            <p>Torpedo attack modifiers (HG p.39): <span className="text-slate-200">DM-2</span> when attacking a target below 2,000 tons (applied automatically from target tonnage). Point Defence against a torpedo salvo has its Effect <span className="text-slate-200">halved (rounded down)</span> before counting torpedoes destroyed — a PD Effect of 3 removes only 1 torpedo.</p>
          ...
          <Sub title="PER-SLOT FIRING LIMIT">
            ...
            <p>The Ship Detail modal labels each slot&apos;s mount type. Turret-mount weapons: <span className="text-slate-200">Single Turret</span> (1), <span className="text-slate-200">Double Turret</span> (2), <span className="text-slate-200">Triple Turret</span> (3), <span className="text-slate-200">Quad Turret</span> (4, HG p.81). A Quad Turret with 4 laser weapons grants <span className="text-slate-200">DM+3</span> on Point Defence — this is computed automatically from weapon count. Barbette and bay weapons (Torpedo, Missile Barbette, Ion Cannon, all *_Barbette, Ion Cannon Bay, Fusion/Meson/Particle/Railgun Bay) are always standalone — labelled <span className="text-slate-200">Barbette</span> or <span className="text-slate-200">Bay</span>, capped at 1 per slot, never combinable into a Quad mount (HG p.29: "a barbette uses a single Hardpoint").</p>
          </Sub>
          ...
          <Sub title="WEAPON RANGE LIMITS">
            ...
            <KV k="Fusion Gun Bay (Small/Medium)" v="Medium (bay, AP 6, Radiation — 6D×10 / 7D×20)" />
            <KV k="Fusion Gun Bay (Large)" v="Long (bay, AP 8, Radiation — 10D×100)" />
            <KV k="Railgun Bay (Small/Medium)" v="Short (bay, AP 10 — 3D×10 / 5D×20)" />
            <KV k="Railgun Bay (Large)" v="Medium (bay, AP 10 — 6D×100)" />
            <KV k="Pulse Laser" v="Long" />
            <KV k="Pulse Laser Barbette" v="Long" />
            <KV k="Meson Gun Bay (all sizes)" v="Long (bay, AP ∞ — ignores all armour — 5D×10 / 6D×20 / 6D×100)" />
            <KV k="Particle Beam" v="Very Long (Radiation)" />
            <KV k="Particle Barbette" v="Very Long (Radiation)" />
            <KV k="Particle Beam Bay (Small/Medium)" v="Very Long (bay, Radiation — 6D×10 / 8D×20)" />
            <KV k="Particle Beam Bay (Large)" v="Distant (bay, Radiation — 10D×100)" />
          ...
          <Sub title="SPECIAL WEAPON MECHANICS">
            <p><span className="text-slate-200">AP (Armour Piercing) trait</span> — reduces effective armour before damage: <code className="text-(--neon-cyan)">effectiveArmour = max(0, armour − apReduction)</code>. Weapons: Railgun AP 4, Fusion Barbette AP 3, Plasma Barbette AP 2, Railgun Barbette AP 5, Fusion Gun Bay AP 6 (Small/Medium) / AP 8 (Large), Railgun Bay AP 10 (all sizes), Meson Gun Bay AP ∞ — ignores all armour (all sizes).</p>
            ...
            <p><span className="text-slate-200">Bay weapons</span> (HG p.31–33) — Fusion Gun, Meson Gun, Particle Beam, and Railgun Bay deal ordinary hull damage (unlike the Ion Cannon Bay): roll + Effect − effective armour, then × the bay's damage multiple (×10 Small, ×20 Medium, ×100 Large). <span className="text-blue-400">Meson Gun Bay</span> carries AP ∞ — it ignores all Armour and radiation shielding outright (HG p.31). <span className="text-slate-200">Bay vs small target DM</span> (HG p.31) — every bay weapon (these four families and Ion Cannon Bay alike) suffers <span className="text-red-400">DM-2</span> vs targets ≤2,000 tons and <span className="text-red-400">DM-4</span> vs targets ≤100 tons, shown automatically as a "Bay vs small target" row in the DM Summary. Thresholds are inclusive, unlike the Torpedo small-ship DM's strict {'<'}2,000 tons. Hardpoint cost: 1 for Small/Medium bays, 5 for Large bays (HG p.31).</p>
        ...
        <Section id="actions" title="Actions Phase (Crew)">
          ...
          <Sub title="AVAILABLE ACTIONS">
            ...
            <KV k="Engineer" v="OVERLOAD M-DRIVE — 2D6 + Engineer, Difficult (10+). Success: +1 Thrust (fixed, not +Effect) next round. Cumulative DM−2 per prior attempt this battle (2nd attempt DM−2, 3rd DM−4, ...) — counter increments on success or failure, no in-app reset (RAW requires offline maintenance). Effect ≤ −6: M-Drive critical Severity 1, applied automatically — never downgrades an existing worse M-Drive critical (CRB p.171). REPAIR SYSTEM — difficulty scales with severity: Avg 8+ (Sev 1–2) / Diff 10+ (Sev 3–4) / Very Diff 12+ (Sev 5–6). GM selects which critical to repair. Removes 1 critical hit (CRB p.167)." />
