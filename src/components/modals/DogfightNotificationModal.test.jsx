...
describe('DogfightNotificationModal — render', () => {
  ...
  it('VERIFICA button disabled until all ships have an intent', () => {
    const { groups } = setupTwoShips()
    render(<DogfightNotificationModal groups={groups} onDone={() => {}} />)
    expect(screen.getByText(/CONFIRM INTENTS/)).toBeDisabled()
  })

  it('VERIFICA enabled once all ships have an intent', () => {
    const { groups } = setupTwoShips()
    render(<DogfightNotificationModal groups={groups} onDone={() => {}} />)
    const [siA, siB] = screen.getAllByText('YES')
    fireEvent.click(siA)
    fireEvent.click(siB)
    expect(screen.getByText(/CONFIRM INTENTS/)).not.toBeDisabled()
  })
})
...
describe('DogfightNotificationModal — all YES → dogfight', () => {
  it('shows DOGFIGHT ATTIVO outcome', () => {
    ...
    fireEvent.click(siA)
    fireEvent.click(siB)
    fireEvent.click(screen.getByText(/CONFIRM INTENTS/))
    expect(screen.getByText(/DOGFIGHT ACTIVE/)).toBeInTheDocument()
  })
...
describe('DogfightNotificationModal — mixed intent → pursuit check', () => {
  it('shows pursuit check section', () => {
    const { groups } = setupTwoShips()
    render(<DogfightNotificationModal groups={groups} onDone={() => {}} />)
    const [siA] = screen.getAllByText('YES')
    const [, noB] = screen.getAllByText('NO')
    fireEvent.click(siA)
    fireEvent.click(noB)
    fireEvent.click(screen.getByText(/CONFIRM INTENTS/))
    expect(screen.getByText(/Pursuit check/i)).toBeInTheDocument()
    expect(screen.getByText(/Pursuer/)).toBeInTheDocument()
